...
class AnalysisSummaryWindow(ctk.CTkToplevel):
    ...
    def create_summary_table(self):
        ...
        main_frame.grid_rowconfigure(0, weight=1)  # Donne de l'espace pour le contenu
        main_frame.grid_rowconfigure(1, weight=0)  # Donne de l'espace pour les boutons
        ...
        if len(self.sample_list) > 1:
            ...
            for col, value in enumerate(avg_row):
                ...
                avg_label.grid(row=0, column=col, padx=5, pady=5, sticky='w')
    
            for col, value in enumerate(std_row):
                std_label = ctk.CTkLabel(stats_frame, text=value)
                std_label.grid(row=1, column=col, padx=5, pady=5, sticky='w')
    
        # Configurer le défilement uniquement pour les données
